...
use fuser::{
    ...
    Request, FUSE_ROOT_ID, MountOption,
};
use sha3::{Sha3_256, Digest};
use serde::{Serialize, Deserialize};

// Wrapper to allow serialization of FileType
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq)]
enum CathedralFileType {
    NamedPipe,
    CharDevice,
    BlockDevice,
    Directory,
...
impl CathedralFileType {
    fn from_fuser(ft: FileType) -> Self {
        match ft {
            FileType::NamedPipe => CathedralFileType::NamedPipe,
            FileType::CharDevice => CathedralFileType::CharDevice,
            FileType::BlockDevice => CathedralFileType::BlockDevice,
            FileType::Directory => CathedralFileType::Directory,
            FileType::RegularFile => CathedralFileType::RegularFile,
            FileType::Symlink => CathedralFileType::Symlink,
            FileType::Socket => CathedralFileType::Socket,
        }
    }

    fn to_fuser(&self) -> FileType {
        match self {
            CathedralFileType::NamedPipe => FileType::NamedPipe,
            CathedralFileType::CharDevice => FileType::CharDevice,
            CathedralFileType::BlockDevice => FileType::BlockDevice,
            CathedralFileType::Directory => FileType::Directory,
            CathedralFileType::RegularFile => FileType::RegularFile,
            CathedralFileType::Symlink => FileType::Symlink,
            CathedralFileType::Socket => FileType::Socket,
        }
    }
}
...
impl CathedralFS {
    pub fn new(theosis_threshold: f64) -> Self {
        ...
        let root = CathedralNode {
            ...
            parent: FUSE_ROOT_ID,
            name: String::from("/"),
            kind: CathedralFileType::Directory,
            size: 0,
            blocks: 0,
            atime: now,
            mtime: now,
    ...
    fn create_node(&mut self, parent: u64, name: &str, kind: FileType, theosis: f64) -> u64 {
        ...
        let node = CathedralNode {
            ...
            parent,
            name: name.to_string(),
            kind: CathedralFileType::from_fuser(kind),
            size: 0,
            blocks: 0,
            atime: now,
            mtime: now,
...
impl Filesystem for CathedralFS {
    fn lookup(&mut self, _req: &Request, parent: u64, name: &OsStr, reply: ReplyEntry) {
        ...
        let mut found_node: Option<CathedralNode> = None;
...
fn main() {
    let mountpoint = std::env::args().nth(1).expect("Usage: catedral-fuse <mountpoint>");

    println!("CATEDRAL-FUSE v1028.3 — Filesystem da Catedral ARKHE");
    println!("Montando em: {}", mountpoint);

    let fs = CathedralFS::new(0.3);  // Threshold de Theosis = 0.3

    let options = vec![
        MountOption::AutoUnmount,
        MountOption::FSName("catedral_fs".to_string()),
    ];
