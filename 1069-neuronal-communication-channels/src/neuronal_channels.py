...
class Neuron:
    ...
    def update(self, dt: float, external_current: float = 0.0) -> bool:
        ...
        if self.V_m > self.threshold:
            ...
            self.spike_times.append(get_current_time())

            # Avisar as sinapses eferentes que houve um spike
...
class Synapse:
    ...
    def compute_current(self, V_post: float) -> float:
        ...
        g = self.w * self.g_max * np.exp(-dt_spike / self.tau_decay) if dt_spike > 0 else 0.0
...
if __name__ == "__main__":
    ...
    # The synapse must be attached to the pre-synaptic neuron so it gets called when pre spikes
    # BUT in this code, 'n1.add_synapse(syn)' makes it act as an incoming synapse for computing current.
    # We should distinguish efferent and afferent synapses in a more complete model.
    # For now, following the original structure:
    n1.add_synapse(syn)

    print(f"Initial weight: {syn.w}")
    # Simula um spike chamando update com corrente externa
    set_current_time(0.0)
    for i in range(100):
        t = i * 0.1
        set_current_time(t)
        spiked = n1.update(0.1, external_current=100.0) # Força um spike
        if spiked:
            print(f"Spike at t={t}, new weight: {syn.w}")
