...
impl GeometricPolicyEngine {
    ...
    pub async fn authorize(
        ...
        let action_vec_storage;
        let action_vec = match action_emb {
            Some(e) => e.view(),
            None => {
                action_vec_storage = self.geometry.embed(action);
                action_vec_storage.view()
            }
        };

        let output_vec_storage;
        let output_vec = match output_emb {
            Some(e) => e.view(),
            None => {
                output_vec_storage = self.geometry.embed(output);
                output_vec_storage.view()
            }
        };
    ...
    async fn measure_representation_diversity(&self, emb: &ArrayView1<'_, f32>) -> f32 {
