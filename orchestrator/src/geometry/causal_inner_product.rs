...
impl CovarianceMatrix {
    pub fn from_vectors(vectors: &[Array1<f32>]) -> Self {
        ...
        for v in vectors {
            let centered = v - &mean;
            cov = cov + centered.view().insert_axis(ndarray::Axis(1)).dot(&centered.view().insert_axis(ndarray::Axis(0)));
        }
        cov /= (n - 1) as f32 ;
        ...
        let cov_na = na::DMatrix::<f64>::from_vec(d, d, cov.iter().map(|&x| x as f64).collect());
        let cov_inv_na = cov_na.clone().try_inverse()
            .unwrap_or_else(|| cov_na.pseudo_inverse(1e-6).unwrap());

        let cov_inv = Array2::from_shape_vec((d, d), cov_inv_na.as_slice().iter().map(|&x| x as f32).collect())
            .unwrap();
