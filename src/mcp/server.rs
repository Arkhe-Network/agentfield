...
pub async fn start_mcp_server(
    ...
    let auth_config = Arc::new(AuthConfig {
        required_token: required_token.unwrap_or_else(|| "".to_string()),
    });
    ...
    let app = Router::new()
        ...
        .layer(axum::middleware::from_fn_with_state(
            auth_config.clone(),
            |axum::extract::State(config): axum::extract::State<Arc<AuthConfig>>, req: axum::extract::Request, next: axum::middleware::Next| async move {
                if config.required_token.is_empty() {
                    // Sem autenticação exigida
