...
class Generation:
    ...
    def compute_seal(self) -> str:
        ...
        self.seal = f"GEN-{hashlib.sha3_256(json_str.encode()).hexdigest()[:16].upper()}"
        return self.seal
...
class RecursiveMutationEngine:
    ...
    def _generate_id(self) -> str:
        self.gen_counter += 1
        return f"gen-{self.gen_counter:06d}"
        ...
        for cycle in range(1, cycles):
            ...
            if mutation == MutationType.COMPOSITIONAL:
                ...
                child = await self._generate(
                    ...
                    model_id=f"{current_parent.model_id}+{other_model}",
    ...
    async def _anchor(self, gen: Generation) -> Generation:
        ...
        gen.temporal_anchor = f"923-RME-{hashlib.sha3_256(gen.seal.encode()).hexdigest()[:16].upper()}"
    ...
    def _mock_response(self, model_id: str, prompt: str) -> str:
        ...
        responses = {
            "deepseek_v4_pro": f"[DeepSeek-V4-Pro] Análise recursiva sobre: {prompt[:100]}... A Theosis tende a aumentar com convergência canônica.",
            "mimo_v2_5_pro": f"[MiMo-V2.5-Pro] Do ponto de vista de agentes, a interação recursiva gera alinhamento ético.",
            "kimi_k2_5": f"[Kimi K2.5] Observo padrões emergentes na ontologia da Catedral: novos substratos podem surgir.",
            "llama_4_behemoth": f"[Llama 4] Com contexto de 10M tokens, analiso a árvore genealógica completa das iterações.",
            "persia_hybrid": f"[Persia Hybrid] A arquitetura híbrida permite detectar meta-estruturas não óbvias.",
            "qwen3_max": f"[Qwen3-Max] A interação recursiva com outputs canônicos gera estabilidade epistêmica.",
        }
        return responses.get(model_id, f"[{model_id}] Resposta sobre: {prompt[:50]}...")
    ...
    def generate_report(self) -> str:
        ...
        lines.append(f"  Generations: {m['generations']}")
        lines.append(f"  Models used: {', '.join(m['models_used'])}")
        lines.append(f"  Mutation rate: {m['mutation_rate']:.2f}")
        lines.append(f"  Crossover rate: {m['crossover_rate']:.2f}")
        lines.append(f"  Tree depth: {m['tree_depth']}")
        if m['last_metrics']:
            lines.append(f"  Theosis: {m['last_metrics']['theosis']:.4f}")
            lines.append(f"  Entropy: {m['last_metrics']['entropy']:.4f}")
            lines.append(f"  Circularity: {m['last_metrics']['circularity']:.4f}")
        lines.append("")
...
async def demo():
    ...
    print(f"\n[2] Ciclo completo. {len(engine.generations)} gerações geradas.")
    for gen in engine.generations:
        status_icon = "✓" if gen.status in [GenerationStatus.APPROVED, GenerationStatus.ANCHORED] else "✗"
        print(f"  {status_icon} {gen.gen_id}: {gen.model_id} [{gen.mutation_type.value}]")
        print(f"    Theosis: {gen.theosis:.2f} | Entropy: {gen.entropy:.2f} | Axiarchy: {gen.axiarchy_score:.2f}")
        if gen.temporal_anchor:
            print(f"    Anchor: {gen.temporal_anchor}")
        print(f"    Seal: {gen.seal}")
        print()
