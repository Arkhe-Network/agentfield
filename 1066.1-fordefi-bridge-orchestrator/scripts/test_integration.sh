...
python -m vault_manager create "BRICS-Treasury" "ethereum,polkadot"
...
EOF
python -m policy_engine apply vault_123 /tmp/test_policy.yaml

# 3. Criar transação
echo "[TEST] Step 3: Criar transação..."
python -m tx_lifecycle create vault_123 0xabc... 1.0 ethereum

# 4. Simular transação
echo "[TEST] Step 4: Simular transação..."
python -m tx_lifecycle simulate tx_123

# 5. Gerar ZK-proof
echo "[TEST] Step 5: Gerar ZK-proof..."
python -m zk_proof_generator generate op_123 vault_create vault_123

# 6. Ancorar na RBB Chain
echo "[TEST] Step 6: Ancorar na RBB Chain..."
python -m rbb_anchor anchor proof_123 0xabc... vault_create vault_123 APPROVED

# 7. Verificar ancoragem
echo "[TEST] Step 7: Verificar ancoragem..."
python -m rbb_anchor verify proof_123 0xabc...

# 8. Atualizar dashboard
echo "[TEST] Step 8: Atualizar Theosis Dashboard..."
python -m theosis_injector update-vault vault_123 1000000.0 0.3 10 ACTIVE

# 9. Criar trigger CARE
echo "[TEST] Step 9: Criar trigger CARE..."
python -m care_bridge create vault_123 "Price Drop" "price_drop>10%" "hedge_via_dex"
