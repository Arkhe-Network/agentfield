...
class TestVaultManager(unittest.TestCase):
    def setUp(self):
        self.patcher = patch('vault_manager.FordefiClient')
        self.MockFordefiClient = self.patcher.start()
        self.mock_client = self.MockFordefiClient.return_value
        self.mock_client.create_vault.return_value = {"id": "vault_123", "created_at": 1234567890}
        self.mock_client.list_vaults.return_value = [{"id": "vault_123", "name": "Test-Vault", "chains": ["ethereum"], "status": "ACTIVE"}]
        self.mock_client.get_vault.return_value = {"status": "ACTIVE", "risk_score": 0.1, "balance_usd": 1000}
...
class TestTransactionLifecycle(unittest.TestCase):
    def setUp(self):
        ...
        self.temp_reg = os.path.join(tempfile.gettempdir(), 'tx_registry_test.json')
        if os.path.exists(self.temp_reg):
            os.remove(self.temp_reg)
        tx_lifecycle.TX_REGISTRY = self.temp_reg

        self.patcher = patch('tx_lifecycle.FordefiClient')
        self.MockClient = self.patcher.start()
        self.mock_client = self.MockClient.return_value
        self.mock_client.create_transaction.return_value = {"id": "tx_123"}
        self.mock_client.simulate_transaction.return_value = {"status": "SIMULATED", "risk_level": "LOW", "gas_estimate": "21000"}
        self.mock_client.submit_transaction.return_value = {"status": "SUBMITTED", "tx_hash": "0x123"}
        self.mock_client.get_transaction.return_value = {"status": "CONFIRMED"}

    def tearDown(self):
        self.patcher.stop()
        import os
        if hasattr(self, 'temp_reg') and os.path.exists(self.temp_reg):
            os.remove(self.temp_reg)
...
class TestCAREBridge(unittest.TestCase):
    def setUp(self):
        import care_bridge
        self.temp_reg = os.path.join(tempfile.gettempdir(), 'care_registry_test.json')
        if os.path.exists(self.temp_reg):
            os.remove(self.temp_reg)
        care_bridge.CARE_REGISTRY = self.temp_reg
        self.patcher = patch('care_bridge.FordefiClient')
        self.MockFordefiClient = self.patcher.start()
        self.mock_client = self.MockFordefiClient.return_value
        self.mock_client.create_care_trigger.return_value = {'id': 'care_123'}
...
class TestTheosisInjector(unittest.TestCase):
    def setUp(self):
        ...
        if os.path.exists(theosis_injector.METRICS_REGISTRY):
            os.remove(theosis_injector.METRICS_REGISTRY)
